...
IMPLEMENTATION = {
    ...
    "GAL": "`frontend/src/render.mjs`, `frontend/src/app.mjs`, `frontend/src/styles.css`, `backend/app/services/generations.py`, `backend/app/services/collections.py`, `backend/app/api/collections.py`",
...
TESTS = {
    ...
    "GAL": "gallery/recall/preferences, collection, query-performance, migration, and lifecycle integration tests; frontend render tests; Playwright",
...
PUBLICATION_ROWS = [
    ...
    ("25", "Authoritative publisher batches remain ordered in history while only the compact final/best binary set is downloadable", "result normalization, queue/asset services, ordered detail API and role-grouped renderer", "multiple-publisher/multiple-batch lifecycle and render tests"),
    ("32", "Transferred ComfyUI output/temp sources are removed through a bounded companion route", "cleanup adapter/worker flow plus `comfyui_extension/cif_artifact_cleanup`", "adapter and lifecycle cleanup/retry tests"),
...
def render() -> str:
    ...
    lines = [
        ...
        "",
        "This matrix is generated by `scripts/generate_traceability.py`. The first table "
        "retains every ID from the historical product v1.0 baseline and maps it to the "
        "current repository. Workflow-integration language in that baseline is superseded "
            "by the publication acceptance matrix that follows; Ollama model-selection, "
            "queued-cancellation, and artifact-retention language are superseded by the current "
            "overrides below. Run `python3 "
        "scripts/generate_traceability.py --check` to detect an omitted or stale row.",
        "",
    ...
    lines.extend(
        [
            "",
            "## Current Ollama router override",
            "",
            "Historical OLL-001 and OLL-004 required an exact server-selected model. The "
            "current deployment configures the Ollama-compatible router base URL and the "
            "stable `CIF_OLLAMA_MODEL` alias (default `nighttime`), verifies that `/api/tags` "
            "advertises that alias without an offline health flag, sends it on every "
            "`/api/chat` attempt, and records the effective "
            "model returned in the response. Thinking defaults on with explicit `think: \"xhigh\"`; "
            "the checkbox inside either collapsed Prompt pre-processor section can disable it "
            "with `think: false`. Each mode exposes editable default instructions with reset, "
            "browser draft persistence, and successful-composition recall. The adapter first "
            "normalizes chat `message.content` and `message.thinking` into its response/thinking "
            "parser, accepts a schema-constrained final object from content, falls back to one "
            "in thinking, and treats absent thinking as a capability warning when the response "
            "is valid. It never promotes unstructured reasoning. Refine mode redraws normalized "
            "unchanged output with a new seed and progressively stronger sampling up to three "
            "candidates, and a successful composition becomes the server-authoritative "
            "positive prompt sent to ComfyUI. Since Ollama shares `num_predict` between thinking "
            "and final output, schema-incomplete `done_reason: length` responses receive bounded "
            "`2048 → 4096 → 8192` escalation with unchanged thinking, schema, instruction, "
            "temperature, and candidate seed; with thinking enabled, a candidate whose three "
            "escalations all end in `length` makes one extra attempt with `think: false` at the "
            "base allowance, and a failed fallback advances to the next candidate, so the "
            "terminal budget error fires only after every candidate is exhausted. Transient "
            "generate failures receive an independent bounded retry. Automatic composition "
            "failures receive bounded browser backoff and "
            "visibly pause with an unchecked switch and retry action on exhaustion. Terminal "
            "failures retain privacy-safe field/status/length/stage/budget diagnostics and the "
            "requested thinking mode without raw reasoning. The application does not moderate or "
            "restrict prompt content. "
            "Optional `CIF_OLLAMA_API_KEY` authentication and all Ollama traffic remain backend-only "
            "through the router.",
            "",
            "## Current queued-cancellation override",
            "",
            "Historical EXEC-015 retained a generation cancelled before dispatch. The current "
            "product instead treats this as cancel-and-delete: the API returns `204`, removes "
            "the generation and recall data, performs normal owned-data cleanup, and emits a "
            "deletion event so every open gallery removes the card. Cancellation after dispatch "
            "begins retains its existing reconciliation and artifact-history behavior. This is "
            "implemented in `backend/app/services/generations.py`, "
            "`backend/app/api/generations.py`, and `frontend/src/app.mjs`, with integration and "
            "Playwright coverage.",
            "",
            "## Current artifact-retention override",
            "",
            "The frontend is authoritative for files produced by its jobs. It keeps the latest "
            "semantic stage while work is active, the authored final batch on success, or one "
            "best eligible image after cancellation/failure/interruption. Complete bounded "
            "history metadata remains available without keeping every binary. After local "
            "processing is durable, the adapter requests deletion of transferred ComfyUI "
            "`output`/`temp` files through the included bounded companion route; failures remain "
            "visible and retry on startup.",
            "",
    ...
    lines.extend(
        [
            ...
            "## Normative boundary",
            "",
            "The ComfyUI publisher/custom-node package, prepared workflows, models, and custom-node dependencies are external prerequisites. This repository discovers and validates committed `<stem>.json` / `<stem>.api.json` / `<stem>.interface.json` publications through network APIs; it does not implement the publisher, install dependencies, mutate publication artifacts, or revive the historical embedded `FrontendWorkflowContract` design. It does ship a narrow companion route that deletes only transferred frontend-generation files from ComfyUI output/temp storage.",
            "",
        ]
    )
    return "\n".join(lines)
