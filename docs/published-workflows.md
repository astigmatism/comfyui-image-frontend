...
The browser selects exactly one workflow for a generation action because prompt construction can be architecture-specific. Within that workflow, users can select one or more published checkpoint values. The checkpoint picker organizes those values into persisted Top picks, Preferred, Occasional, and Unsorted tiers; new published values append to Unsorted and removed values are ignored. Every selected checkpoint is still separately validated, accepted, persisted, and displayed as its own generation, and each request contains one `source_key`, one immutable revision, and one scalar checkpoint value.
...
- the compact application-owned image set in `artifacts`: the final batch on success or one best available image after cancellation/failure;
- durable lifecycle `events`.

CIF publisher metadata is read from the top-level list-shaped `comfyui_image_frontend` payload (with legacy nested shapes accepted for stored-history compatibility) and verified against the frozen declaration. Its `artifacts` list is authoritative for batch membership, `batch_index`, and order; the ordinary `images` field stays in raw history and is not counted a second time. Unknown or mismatched publisher metadata is recorded as an error, not silently promoted. Native file references are restricted to ComfyUI's `filename` / `subfolder` / `type` tuple with types `input`, `output`, or `temp`; arbitrary paths are never accepted. Retrieved bytes are size-limited and first archived in owner-scoped application storage. Advancing stages prune older originals/thumbnails; terminal success keeps the final batch and cancellation/failure keeps one best image. The frontend then deletes successfully transferred `output`/`temp` sources through its ComfyUI companion route. Cleanup failures remain visible and retryable. A retrieval failure leaves the logical locator in declared/unmapped/raw result data and reports a partial-output warning or required-final failure as appropriate.

The gallery uses the authored `final` result as its primary image when available. Every retained final-batch sibling remains independently inspectable and downloadable. Earlier stages and native results remain inspectable as JSON history metadata but are not kept as downloadable binary duplicates.
...
7. Start with a low-cost request, then inspect `prompt_id`, effective parameters, raw history, declared/unmapped outputs, the compact retained artifact set, and successful ComfyUI source cleanup.
