...
- Source revision identity is publication UUID plus the manifest-recorded editable-workflow SHA-256 and exact observed API/manifest SHA-256 values; recorded API hash drift is retained as a warning diagnostic.
- Generation results retain native prompt ID, complete server-side history/status, effective parameters, manifest-ordered publisher outputs with authoritative batch indices, untouched node-keyed nonpublisher outputs, and warnings/errors. Binary retention is compact: success keeps the final batch, cancellation/failure keeps one best image, and pruned images remain metadata-only in history. Successfully transferred ComfyUI `output`/`temp` files are removed through the companion cleanup route. Public history removes top-level submitted prompt/extra-data graph envelopes while preserving actual node results, publisher metadata, custom UI fields, raw status/errors, and execution metadata.
...
- Publication v1 also accepts required `image` inputs bound to `CIFImageParameter.image`. Clients submit an application-owned opaque asset ID, never a ComfyUI filename, path, locator, or remote URL. Image-free publications remain valid without migration.
- A later normal ComfyUI save or mixed bundle may change editable/API bytes. Discovery preserves an otherwise valid adjacent bundle with explicit workflow/API drift warnings; structural graph, node-count, manifest/interface, and dependency checks remain hard requirements, and execution is identified by the observed API hash.
- Publication v1 may add compiled `generation_source` and `technical_inventory` sections. Recognized v1 data is returned on source summary/detail APIs without becoming request inputs; older manifests remain valid with unavailable metadata, and unknown values/entries/warnings/extra fields are retained.
