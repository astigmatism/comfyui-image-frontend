...
test("recently used resolutions record on commit, restore on click, persist, and scope per source", async ({
  ...
  // Wait for server acknowledgement before verifying cross-session persistence.
...
// Server automation is covered by server-auto-generation.spec.mjs and backend coordinator tests.
