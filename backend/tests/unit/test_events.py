...
def test_event_broker_delivers_only_to_the_subscribed_owner() -> None:
    async def scenario() -> None:
        broker = EventBroker()
        async with broker.subscribe("owner-a") as queue_a:
            async with broker.subscribe("owner-b") as queue_b:
                await broker.publish("owner-a", {"type": "generation.running"})
                assert (await asyncio.wait_for(queue_a.get(), timeout=0.1))["type"] == "generation.running"
                try:
