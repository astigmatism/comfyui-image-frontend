...
The Prompt Assistant uses the router's native Ollama API with no authentication, sends `think: true` on every composition request, and does not use the OpenAI-compatible `/v1` API. Do not set `CIF_OLLAMA_MODEL`: the backend omits the model from `/api/generate`, allowing this active-only router to select `hauhau-qwen3.6-35b-a3b-aggressive-q4-k-m:qwen35-parser`, and records the effective model returned in the response. Create mode retries an exact duplicate with explicit distinct-result guidance and different sampling so reapplying the same Creative Direction still produces a new prompt. The operator-only router dashboard is `http://192.168.1.21:11435/`; it is not exposed in the application UI. Keep both `/api/tags` and `/api/generate` routed through the router, never its private upstream Ollama container.
