...
Every configured instance supplies its own publication catalog, diagnostics, execution health, and queue capacity. The workflow picker shows each source once, preferring the default instance for images and `CIF_COMFYUI_TEXT_INSTANCE_ID` for text. Copies retain independent source keys and revisions, exposed through safe `replicas` metadata. The image runtime defaults to `CIF_COMFYUI_DEFAULT_INSTANCE_ID`; the separate Prompt runtime selector overrides the text default per request. A text target must hold the exact requested publication revision. See [CPU prompt-generation deployment](docs/comfyui-promptgen.md) for configuration and bundle updates.
