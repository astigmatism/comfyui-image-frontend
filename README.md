...
2. The standard Compose deployment automatically adds the household RTX 3080 worker to the existing RTX 3090 primary through `deployment/comfyui-instances.env`. For another topology, override `CIF_COMFYUI_INSTANCES` in the private `.env`; optionally set `CIF_COMFYUI_DEFAULT_INSTANCE_ID` when the first entry should not be the default. Reuse the primary server's existing stable `CIF_COMFYUI_INSTANCE_ID` value as that entry's `id`; changing it changes every published source key.
...
Application code and the built browser assets live in the frontend image. Compose reads the tracked non-secret runtime defaults followed by the private `.env` when it creates the frontend container. Enabling or changing the instance list therefore requires rebuilding and recreating/restarting only `comfyui-image-frontend`. Never restart or recreate either ComfyUI container for this application update.

The updater intentionally never copies `.env.example` over the deployment's persistent, gitignored `.env`. Instead, `compose.example.yml` loads `deployment/comfyui-instances.env` first and `.env` last. The tracked file supplies safe RTX 3090 display text and `CIF_COMFYUI_ADDITIONAL_INSTANCES` for Worker 1; the backend appends that worker to the primary ID, URL, user, and concurrency already stored in the private file. An existing deployment therefore receives both household runtimes automatically on its next `./update_and_restart` without changing its stable primary identity. A full private `CIF_COMFYUI_INSTANCES` list remains authoritative and suppresses the additional-instance layer.
...
When both instance arrays are absent—for example, in a custom Compose deployment that does not load the standard defaults—the application constructs one compatible entry named **Original** from `CIF_COMFYUI_BASE_URL`, optional `CIF_COMFYUI_WS_URL`, `CIF_COMFYUI_INSTANCE_ID`, optional `CIF_COMFYUI_USER`, and `CIF_COMFYUI_CONCURRENCY`. The selector shows a configuration warning icon in this one-runtime legacy mode. This preserves existing single-instance deployments without presenting the internal ID as a display name. Do not set a list entry URL containing credentials; connection URLs and optional user selectors remain server-side and are never returned to the browser.
