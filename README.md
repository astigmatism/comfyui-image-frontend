...
A successful composition ID makes the stored assistant output authoritative for the positive prompt compiled and submitted to ComfyUI. Each composition starts with a fresh cryptographically random candidate seed. Refine retries an unchanged candidate with the next seed and higher temperature. Create rejects the current prompt, recent successful results for the same user and Creative Direction, and the Creative Direction itself, and retries a duplicate with the next seed while keeping the same minimal instruction. The application does not moderate or restrict prompt content. With Auto-generate and Use Creative Direction enabled and a nonempty direction, each image batch uses a fresh Prompt Assistant composition. As soon as every image in the batch is accepted into the application queue, the browser prepares one prompt ahead while those images generate. The result appears immediately in the editable Prompt field with its green ready indicator; the next batch waits until all pending generations finish, reusing that preparation without another LLM request. A matching manual application may also prepare the imminent cycle. Disabling either control discards automatic preparation, while already queued images continue. Retryable automatic composition failures receive at most three browser composition attempts with 1-second then 2-second backoff. Prompt, Creative Direction, instruction, mode, thinking, or source/revision changes discard stale preparation and retry timers; image-only settings reuse a valid prompt. Superseded automatic requests finish without applying their results before the latest inputs are composed. Success resets backoff. Exhaustion or a non-retryable error unchecks Auto-generate and shows an accessible paused message with **Retry Auto-generate**, so the control cannot remain checked but inert. Across frontend recovery, repeated pure output-budget exhaustion is capped at 108 upstream requests when every transport attempt is also used (three browser composition attempts × 36 upstream requests). The operator-only router dashboard is `http://192.168.1.21:11435/`; it is not exposed in the application UI. Keep both `/api/tags` and `/api/chat` routed through the router, never its private upstream LLM service.
...
Auto generation is a persistent per-user controller. Enabling it captures the workflow revision, parameters, checkpoint selection/order, quantity, seed policy, execution runtime, destination folder, input uploads, and effective Creative Direction instructions. It keeps producing batches without a browser and resumes after server recovery. Republished workflows do not change that captured revision. Editing shared controls changes the editor draft; **Apply to auto generation** updates future batches explicitly. Automatic Refine prompts evolve from the last accepted prompt without changing the editor draft.

While enabled, **Stop after** defaults to **200** generation jobs. Quantity × selected checkpoints consumes the allowance, and a final batch is shortened to the exact remainder. Changing the limit starts a fresh allowance for subsequently accepted jobs; already accepted jobs do not consume the new allowance. Blank means unlimited. Reaching the limit disables automation on the server and lets the final accepted jobs finish. Switching the toggle off manually instead removes waiting automatic jobs and lets dispatched jobs finish. Manual work takes priority over queued automatic work without bypassing fairness across accounts.

Temporary Prompt Assistant/runtime outages retain the enabled setting and retry with backoff capped at 60 seconds. Invalid configurations and failed executions stop further production in a visible blocked state. Retry or Apply resumes it after correction. Deleted destination folders block generation instead of redirecting output. Account deletion stops its automation before cleanup.
