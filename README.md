...
The updater intentionally never copies `.env.example` over the deployment's persistent, gitignored `.env`. The image-bundled defaults supply the safe **Primary** label, the existing secondary entry, and the CPU `promptgen` entry; the backend appends those instances while retaining the primary ID, URL, user, and concurrency supplied by the private environment. `compose.example.yml` loads those instances and the CPU assignment from `deployment/comfyui-instances.env`. Custom Compose and direct-container launches supply stage assignments through their private environment; the image does not impose a text assignment on a private single-instance list. Legacy primary identities remain stable. The existing `worker-2` LAN entry is separate from the CPU prompt service and is not an execution choice. A full private `CIF_COMFYUI_INSTANCES` list remains authoritative and suppresses the additional-instance layer; setting `CIF_COMFYUI_ADDITIONAL_INSTANCES=[]` disables bundled additions. An absent text assignment means image-only operation; use `CIF_COMFYUI_TEXT_INSTANCE_ID=` to override the example Compose assignment explicitly. A private full list must contain its assigned text service before upgrading. Before reporting success, `update_and_restart` verifies the running frontend resolved an explicit runtime configuration.
...
For the fixed GPU/CPU deployment, use the shared `comfyui_default` network and the
configuration in [the CPU deployment runbook](docs/comfyui-promptgen.md). The explicit
assignments are `CIF_COMFYUI_DEFAULT_INSTANCE_ID=primary` and
`CIF_COMFYUI_TEXT_INSTANCE_ID=promptgen`. Preserve an existing primary's stable ID
when adapting another deployment; do not replace the production Compose file with
...
**Generate** always uses the server-assigned GPU ComfyUI. **Generate prompt** always uses the assigned CPU ComfyUI. There are no runtime selectors. New API requests omit `comfyui_instance_id`; legacy requests supplying a different target are rejected. Accepted jobs record their runtime, and uploads, submission, progress, history, results, and cancellation continue through that adapter. Automation normalizes the assignments before future cycles while preserving accepted work. Historical runtime labels remain available in records.
...
submissions across reloads and automatically checks receipts before replaying the original
request with the same key. Reconnection appears inside the originating Generate button. See
