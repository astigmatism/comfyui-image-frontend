...
The Prompt Assistant uses the router's native Ollama API with no authentication and does not use the OpenAI-compatible `/v1` API. Thinking defaults on; the focused prompt editor exposes a **Thinking mode** checkbox beside the Creative Direction mode choices and forwards its value as top-level `think`. The compact sidebar and Auto-generate paths continue to use thinking. When thinking is enabled, the backend requires nonempty thinking output so the setting cannot be silently ignored; when disabled, it accepts the structured final prompt from the ordinary response. Do not set `CIF_OLLAMA_MODEL`: the backend omits the model from `/api/generate`, allowing this active-only router to select `hauhau-qwen3.6-35b-a3b-aggressive-q4-k-m:qwen35-parser`, and records the effective model returned in the response. Transient `/api/generate` responses (`408`, `425`, `429`, and `5xx`), connection failures, and malformed JSON are retried at most twice with short backoff; terminal failures retain safe operation, failure-kind, attempt-count, thinking-mode, and upstream-status diagnostics without storing prompt text in logs. Create mode starts each composition with a fresh cryptographically random sampling seed, rejects recent results for the same user and Creative Direction, and retries a duplicate with a different seed while keeping the same minimal instruction. With Auto-generate enabled, every generation cycle with a nonempty Creative Direction waits for a fresh Prompt Assistant composition before queueing; a manual application may prepare the imminent cycle, and that preparation is consumed when its generation is accepted. The operator-only router dashboard is `http://192.168.1.21:11435/`; it is not exposed in the application UI. Keep both `/api/tags` and `/api/generate` routed through the router, never its private upstream Ollama container.
