...
The Prompt Assistant uses the router's native Ollama API with no authentication and does not use the OpenAI-compatible `/v1` API. Thinking defaults on; the focused prompt editor exposes a **Thinking mode** checkbox beside the Creative Direction mode choices and forwards its value as top-level `think`. The compact sidebar and Auto-generate paths continue to use thinking. The adapter accepts a schema-constrained final prompt from `response` first and falls back to a schema-constrained object in `thinking`; a missing `thinking` field is recorded as a capability warning when `response` is valid. Refine mode rejects normalized unchanged output with an actionable error. Create mode treats a candidate that only repeats or truncates the Creative Direction (case- and whitespace-insensitive) as a degenerate echo, rejects it, and redraws with the next seed; if every candidate fails to expand the direction the request fails with `prompt_creation_unchanged` (HTTP 422) instead of storing the echo as a successful run. Do not set `CIF_OLLAMA_MODEL`: the backend omits the model from `/api/generate`, allowing this active-only router to select `hauhau-qwen3.6-35b-a3b-aggressive-q4-k-m:qwen35-parser`, and records the effective model returned in the response.

Ollama applies `num_predict` to thinking and final output together; it does not reserve a separate final-answer allowance. Prompt composition therefore uses the named output-budget schedule `2048 → 4096 → 8192`. When `done_reason` is `length` and neither `response` nor the parser-compatible `thinking` field contains a complete `{"prompt": <nonempty string>}` object, the adapter retries at the next allowance while preserving `think`, schema, instruction, temperature, and candidate seed. A complete structured prompt remains acceptable even if the terminal reason is `length`. Exhaustion returns `ollama_output_budget_exhausted` with HTTP 503 and privacy-safe attempt/allowance diagnostics. Transient `/api/generate` responses (`408`, `425`, `429`, and `5xx`), connection failures, and malformed JSON retain their independent at-most-three-attempt transport policy. Create distinctness also remains independent: only a complete duplicate advances its seed and temperature. The absolute backend bound is three budgets × three transport attempts × three Create candidates (27 upstream requests); pure budget exhaustion stops the current candidate after at most nine requests including transport retries.

A successful composition ID makes the stored assistant output authoritative for the positive prompt compiled and submitted to ComfyUI. Create mode starts each composition with a fresh cryptographically random sampling seed, rejects the current prompt, recent successful results for the same user and Creative Direction, and the Creative Direction itself, and retries a duplicate with a different seed while keeping the same minimal instruction. The application does not moderate or restrict prompt content. With Auto-generate enabled, every generation cycle with a nonempty Creative Direction waits for a fresh Prompt Assistant composition before queueing; a manual application may prepare the imminent cycle, and that preparation is consumed when its generation is accepted. Retryable automatic composition failures receive at most three browser composition attempts with 1-second then 2-second backoff. Input/source/revision/runtime changes cancel stale timers and start a fresh cycle; success resets backoff. Exhaustion or a non-retryable error unchecks Auto-generate and shows an accessible paused message with **Retry Auto-generate**, so the control cannot remain checked but inert. Across frontend recovery, repeated pure output-budget exhaustion is capped at 27 upstream requests when every transport attempt is also used. The operator-only router dashboard is `http://192.168.1.21:11435/`; it is not exposed in the application UI. Keep both `/api/tags` and `/api/generate` routed through the router, never its private upstream Ollama container.
